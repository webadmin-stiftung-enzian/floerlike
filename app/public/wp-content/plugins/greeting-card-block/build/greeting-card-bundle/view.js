...
(() => {
/*!******************************************!*\
  !*** ./src/greeting-card-bundle/view.js ***!
  \******************************************/
__webpack_require__.r(__webpack_exports__);
...
/* harmony import */ var _wordpress_interactivity__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! @wordpress/interactivity */ "@wordpress/interactivity");
// import Swiper JS
 ...
 */
const PRODUCTS_STORE_LOCK = 'I acknowledge that using a private store means my plugin will inevitably break on the next store release.';
...
function variationMatches(variation, attributes) {
  return Object.entries(variation.attributes).every(([name, value]) => value === '' || !attributes[name] || attributes[name] === value);
}
...
const {
  state
} = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_5__.store)('greeting-card-bundle', {
  state: {
    get cardMissing() {
    ...
    get matchedVariation() {
      ...
      return state.variationSelector.variations.find(variation => Object.entries(variation.attributes).every(([name, value]) => value === '' || value === state.selectedAttributes[name])) || null;
    },
    ...
    get variationMissing() {
      ...
      return !state.variationSelector.attributes.every(attribute => state.selectedAttributes[attribute.name]);
    },
    ...
    get matchedVariationUnavailable() {
      ...
      return !variation || !variation.inStock || !variation.purchasable;
    },
    get isValid() {
      return !state.cardMissing && !state.textMissing && !state.variationMissing && !state.matchedVariationUnavailable;
    },
    get showCardError() {
    ...
    get isCardPressed() {
      return (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_5__.getContext)().cardId === state.selectedCardId;
    },
    ...
    get isOptionSelected() {
      const {
        attributeName,
        value
      } = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_5__.getContext)();
    ...
    get isOptionAvailable() {
      ...
      return state.variationSelector.variations.some(variation => variation.inStock && variation.purchasable && variationMatches(variation, candidate));
    },
    get charCounter() {
  ...
  actions: {
    ...
    async addToCart() {
      ...
      try {
        ...
        const parentId = state.variationSelector ? state.matchedVariation.variationId : state.productId;
        ...
        const failed = (payload?.responses || []).find(item => item?.status && item.status >= 400);
        if (failed) {
          throw new Error(failed.body?.message || 'Add to cart failed');
        }
  ...
  callbacks: {
    ...
    syncNativePrice() {
      ...
      const variation = state.variationMissing ? null : state.matchedVariation;
      try {
        const {
          state: productsState
        } = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_5__.store)('woocommerce/products', {}, {
          lock: PRODUCTS_STORE_LOCK
        });
        productsState.variationId = variation ? variation.variationId : null;
      } catch (error) {
    ...
    initSwiper() {
      const {
        ref
      } = (0,_wordpress_interactivity__WEBPACK_IMPORTED_MODULE_5__.getElement)();
      new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](ref, {
