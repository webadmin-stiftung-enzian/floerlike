...
> 5. §11/§12: Neuer Testfall (Server-Validierung Max Bundle Size) und
>    aktualisierte Verifikationspunkte.
...
├── build/
├── includes/
│   └── class-integration.php   ← nur: Grusstext-Persistenz (falls nötig, §9) + Admin-Hinweise
...
if (! $bundle || ! $bundle->is_type('bundle')) {
    return; // Block nur auf Bundle-Produkten sinnvoll
}
...
### 7.5 Editor-seitige Ersetzung der Bundle-Form

Damit auf der Produktseite **nicht** zusätzlich die native Bundle-Add-to-Cart-Form
erscheint (Doppelung), wird im Single-Product-Template der Core-/Bundle-Add-to-Cart-
Block entfernt und durch unseren Block ersetzt (siehe §10).
...
## 10. Template-Einbindung

1. Single-Product-Template im Site Editor öffnen.
2. Die native **Add-to-Cart-/Bundle-Form** für Bundle-Produkte entfernen bzw.
   ausblenden (sonst doppelte Add-to-Cart-UI).
3. Unseren Block „Grusskarte + Bundle in den Warenkorb" an dessen Stelle einfügen.

> **Hintergrund:** Product Bundles bringt eine eigene Add-to-Cart-Form mit
> (Auswahl der gebündelten Items). Unser Block **ersetzt deren Darstellung** durch
> die Slider-UI. Es darf pro Seite nur **eine** Add-to-Cart-Mechanik aktiv sein.
>
> **Hinweis für Block Themes (neu in v3.1):** Die PB-Option „Form Location"
...
> Ersetzung findet also ausschliesslich auf Block-Ebene im Single-Product-
> Template statt; „Form Location" spielt in diesem Setup keine Rolle.

**Fertig, wenn:** Die Produktseite zeigt nur unsere Slider-UI und genau einen
